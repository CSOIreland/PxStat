...
BEGIN
	...
	SELECT DISTINCT GMP_CODE AS GmpCode
			...
			,MTR_ID 
			,CASE WHEN MTR_ID IS NULL THEN 0 ELSE 1 END AS HasTable
		into #MtrCls
		FROM TD_GEOMAP
		INNER JOIN TD_GEOLAYER ON GMP_GLR_ID = GLR_ID
		LEFT JOIN TD_CLASSIFICATION ON (@UrlStub + GMP_CODE) = CLS_GEO_URL
		LEFT JOIN TD_MATRIX ON CLS_MTR_ID = MTR_ID
