...
app.performance.ajax.read = function (data) {
    ...
    app.performance.ajax.readRequestsPerSecond();
    app.performance.ajax.readRequestsQueued();
};
//#endregion
...
app.performance.ajax.readRequestsPerSecond = function () {
    var datePicker = $("#performance-input").find("[name=input-date-range]").data('daterangepicker');
    api.ajax.jsonrpc.request(app.config.url.api.jsonrpc.private,
        "PxStat.Security.Performance_API.ReadPrfRequestPerSecond",
        {
            "PrfDatetimeStart": moment(datePicker.startDate).format(app.config.mask.datetime.ajax),
            "PrfDatetimeEnd": moment(datePicker.endDate).format(app.config.mask.datetime.ajax)
        },
        "app.performance.callback.readRequestsPerSecond",
        null,
        null,
        null,
        { async: false });
}
app.performance.callback.readRequestsPerSecond = function (response) {
    $("#performance-charts").find("[name=chart-requests-per-second]").empty();
    $("#performance-charts").find("[name=chart-requests-per-second]").append(
        $("<canvas>", {
            "name": "chart-requests-per-second-canvas",
            "style": "width: 100%; height: 400px"
        })
    );
    var localOptions = $.extend(true, {}, app.performance.chartOptions);
    ...
    new Chart($("#performance-charts").find("[name=chart-requests-per-second-canvas]"), chartOptions);

}

app.performance.ajax.readRequestsQueued = function () {
    var datePicker = $("#performance-input").find("[name=input-date-range]").data('daterangepicker');
    api.ajax.jsonrpc.request(app.config.url.api.jsonrpc.private,
        "PxStat.Security.Performance_API.ReadPrfRequestQueue",
        {
            "PrfDatetimeStart": moment(datePicker.startDate).format(app.config.mask.datetime.ajax),
            "PrfDatetimeEnd": moment(datePicker.endDate).format(app.config.mask.datetime.ajax)
        },
        "app.performance.callback.readRequestsQueued",
        null,
        null,
        null,
        { async: false });
}
app.performance.callback.readRequestsQueued = function (response) {
    $("#performance-charts").find("[name=chart-requests-queued]").empty();
    $("#performance-charts").find("[name=chart-requests-queued]").append(
        $("<canvas>", {
            "name": "chart-requests-queued-canvas",
            "style": "width: 100%; height: 400px"
        })
    );
    var localOptions = $.extend(true, {}, app.performance.chartOptions);
    ...
    new Chart($("#performance-charts").find("[name=chart-requests-queued-canvas]"), chartOptions);
}
