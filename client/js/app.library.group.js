 ...
 * @param {*} response
 */
app.library.group.modal.callback.read = function (response) {
  if (response.error) {
    ...
    $("#modal-read-group").find("[name=grp-code]").empty().text(response.data.GrpCode);
    $("#modal-read-group").find("[name=grp-name]").empty().text(response.data.GrpName);
    $("#modal-read-group").find("[name=grp-contact-name]").empty().text(response.data.GrpContactName);
    $("#modal-read-group").find("[name=grp-contact-email]").empty().html(app.library.html.email(response.data.GrpContactEmail));
    $("#modal-read-group").find("[name=grp-contact-phone]").empty().html(app.library.html.email(response.data.GrpContactPhone));

    // Get data from API
    api.ajax.jsonrpc.request(
      app.config.url.api.private,
      "PxStat.Security.GroupAccount_API.Read",
      { "GrpCode": response.data.GrpCode },
...
app.library.group.modal.callback.readAccountList = function (response) {
  if (response.error) {
    api.modal.error(response.error.message);
