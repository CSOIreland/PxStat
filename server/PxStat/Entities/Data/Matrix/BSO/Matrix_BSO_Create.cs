    ...
    internal class Matrix_BSO_Create : BaseTemplate_Create<PxUpload_DTO, PxUploadValidator>
    {
        ...
        protected override bool Execute()
        {
            ...
            if (latestRelease != null)
            {
                ...
                MemCacheD.CasRepositoryFlush(Resources.Constants.C_CAS_DATA_READ_PRE_DATASET + latestRelease.RlsCode);
                MemCacheD.CasRepositoryFlush(Resources.Constants.C_CAS_DATA_READ_PRE_METADATA + latestRelease.RlsCode);
            }
            else
            {
            ...
            swMatrix.Stop();
            Log.Instance.Info(string.Format("Matrix object created in {0} seconds", Math.Round((double)swMatrix.ElapsedMilliseconds)));
            ...
            swLoad.Stop();
            Log.Instance.Info(string.Format("Matrix loaded in DB in {0} seconds", Math.Round((double)swLoad.ElapsedMilliseconds)));
