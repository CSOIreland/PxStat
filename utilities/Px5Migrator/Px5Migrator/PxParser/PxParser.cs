...
namespace Px5Migrator
{
    ...
    public class PxParser
    {
        ...
        internal static void ThrowParserException(ParseError<char> error)
        {
            ...
            if (error.EOF) unexpected = "Error eof";


            sbLogs.AppendLine("Error parsing");

            if (error.Unexpected != null)
            {
                sbLogs.AppendLine("Error parsing unexpected ");
            }
            if (error.Expected != null)
            {
                sbLogs.AppendLine("Error parsing expected");
            }
            Log.Instance.Debug(new Exception(sbLogs.ToString()));



            if (error.EOF) unexpected = "Error parsing eof";
