...
namespace Px5Migrator
{
    ...
    public class Dmatrix : IDmatrix
    {
        ...
        //The individual langauge specific properties of the matrix. There will be one spec per language and at lease one in the matrix
        ...
        {
            LoadMatrixPropertiesFromPxDocument(document, dto, ado, metaData);
        }
        ...
        public void LoadMatrixPropertiesFromPxDocument(IDocument document, IUpload_DTO dto, IADO ado, IMetaData metaData)
        {
            ...
            // The Main language of the Px File (other languages might exist but for migration we're only concerned with the language on the matrix table)
            Language = document.GetStringElementValue("LANGUAGE");


            // Language = dto.LngIsoCode;

            string matrixEntryLanguage = dto.LngIsoCode;
            if (dto.LngIsoCode == Language) dto.LngIsoCode = null;

            Dspecs = new Dictionary<string, Dspec>();

            var mainSpec = new Dspec(document, ConvertFactory.Convert(domain), dto, metaData);

            mainSpec.Language = matrixEntryLanguage;
